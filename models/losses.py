...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target):
        ...
        realfake_tensor = self.Tensor(input.size()).fill_(target)
        self.realfake_label_var = Variable(realfake_tensor, requires_grad=False)
        target_tensor = self.realfake_label_var
...
class DiscLoss(nn.Module):
    ...
    def get_darkchannl(self, img):
        ...
        #################################
        # 计算梯度图
        new_img = torch.zeros(1, 3, 256, 256)
        new_img[0, :, :, 0] = img[0, :, :, 1] - img[0, :, :, 0]
        rowright2_img = torch.roll(img.clone(), 2, 3)
        new_img[0,:,:,1:255] = ((img - rowright2_img)[0, :, :, 2:])/2.0
        new_img[0, :, :, 255] = img[0, :, :, 255] - img[0, :, :, 254]

        new_img = (new_img + 1) / 2.0  # 梯度完归一化完毕
        #################################
        ...
        for m in range(M):
            for n in range(N):
                patch = new_img[0, :, m:m + patch_size, n:n + patch_size]
                tmp_J = patch.min()
                J[0, m, n] = tmp_J  # J为处理完毕的暗通道
        #################################

        #################################
        # 统计暗通道的像素值
        static = torch.zeros(200, 2)
        tmp_index = 0
        for tmp in range(0, 1000, 5):
            static[tmp_index, 0] = tmp / 1000.0
            tmp_index = tmp_index + 1

        for mm in range(M):
            for nn in range(N):
                abs_tmp = torch.abs(J[0, mm, nn] - static[:, 0])
                min_index = torch.min(abs_tmp, 0)[1]
                static[min_index, 1] = static[min_index, 1] + 1
        #################################

        #################################
        # 计算均值，方差
        # my_mean = torch.sum( static[:,0] * (static[:,1] / torch.sum(static[:,1])))
        my_std = torch.std(static[:, 1])

        my_std = (my_std/256*256)*0.001*0.5
        print(my_std)
        #################################
    ...
    def get_loss(self, net, fakeB, realB):
        ...
        self.loss_D_fake = self.criterionGAN(self.pred_fake, self.get_darkchannl(fakeB))


        # Real
        self.pred_real = net.forward(realB)
        self.loss_D_real = self.criterionGAN(self.pred_real, self.get_darkchannl(realB))
...
class DiscLossWGANGP(DiscLossLS):
    ...
    def get_loss(self, net, fakeB, realB):
        ...
        # writer_D = SummaryWriter(comment='netD')
