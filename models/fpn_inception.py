...
class FPN(nn.Module):
    ...
    def __init__(self, norm_layer, num_filters=256):
        """Creates an `FPN` instance for feature extraction.
            创建一个“ FPN”实例以进行特征提取。
        Args:
          num_filters: the number of filters in each output pyramid level 每个输出金字塔等级中的过滤器数
          pretrained: use ImageNet pre-trained backbone feature extractor 使用ImageNet预训练的骨干特征提取器
        """
